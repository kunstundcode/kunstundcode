...
let codekuenste = [
  ...
  // },
  {
    ...
    userarts: [],
    code: 'function dataURLtoFile(e,o){let t=e.split(","),a=t[0].match(/:(.*?);/)[1],n=atob(t[1]),r=n.length,i=new Uint8Array(r);for(;r--;)i[r]=n.charCodeAt(r);return new File([i],o,{type:a})}let s=function(e){var o,t,a=[],n=[],r=!1,i=!1;e.setup=function(){e.createCanvas(e.windowWidth,e.windowHeight),o=e.width/2,t=e.height/2;for(var r=e.radians(24),i=0;i<15;i++)a.push(150*e.cos(r*i)),n.push(150*e.sin(r*i));e.stroke(0,50),e.strokeWeight(.75),e.background(255)},e.draw=function(){o+=.01*(e.mouseX-o),t+=.01*(e.mouseY-t);for(var i=0;i<15;i++)a[i]+=e.random(-2,2),n[i]+=e.random(-2,2);r?e.fill(e.random(255)):e.noFill(),e.beginShape(),e.curveVertex(a[14]+o,n[14]+t);for(i=0;i<15;i++)e.curveVertex(a[i]+o,n[i]+t);e.curveVertex(a[0]+o,n[0]+t),e.curveVertex(a[1]+o,n[1]+t),e.endShape()},e.mousePressed=function(){o=e.mouseX,t=e.mouseY;for(var r=e.radians(24),i=(e.random(.5,1),0);i<15;i++)a[i]=150*e.cos(r*i),n[i]=150*e.sin(r*i)},e.keyPressed=function(){if("s"===e.key||"S"===e.key){let o=dataURLtoFile(e.canvas.toDataURL("image/png"),Date.now()),t="";t=window.location.origin.includes("localhost")?"http://localhost:5000/api/uploadPicture/"+projectcode:window.location.origin+"/api/uploadPicture/"+projectcode;let a=new FormData;a.append("file",o);let n={async:!0,crossDomain:!0,url:t,method:"POST",headers:{"cache-control":"no-cache","Postman-Token":"c398ba57-8976-47e9-acc1-ec8e4c121b09"},processData:!1,contentType:!1,mimeType:"multipart/form-data",data:a,xhrFields:{withCredentials:!0}};$.ajax(n).done(function(e){console.log(e)})}e.keyCode!=e.DELETE&&e.keyCode!=e.BACKSPACE||e.background(255),"1"==e.key&&(r=!1),"2"==e.key&&(r=!0),"f"!=e.key&&"F"!=e.key||(i=!i),i?e.noLoop():e.loop()}},myp5=new p5(s,document.getElementById("box"));'
  }
];
